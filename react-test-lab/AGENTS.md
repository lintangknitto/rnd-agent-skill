...
Locations (same skill set on each platform):
...
| Agent | Cursor | Claude Code | Codex CLI | When / why |
|-------|--------|-------------|-----------|------------|
| `reviewer` | `.cursor/agents/reviewer.md` | `.claude/agents/reviewer.md` | `.codex/agents/reviewer.toml` | Reviewer independen, dipanggil proaktif saat sesi/fitur dinyatakan selesai atau saat diminta review diff (delegasi: `code-review-and-quality`) |
| `qa-engineer` | `.cursor/agents/qa-engineer.md` | `.claude/agents/qa-engineer.md` | `.codex/agents/qa-engineer.toml` | Rencanakan lalu bangun test coverage: selalu mulai dari `test-case-matrix`, baru implementasi via skill testing yang sesuai layer |
